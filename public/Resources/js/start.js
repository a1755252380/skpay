...
// 监听目标元素
const targetDiv = document.getElementById("loader-wrapper");
const observer = new MutationObserver((mutationsList) => {
  mutationsList.forEach((mutation) => {
    if (mutation.attributeName === "class") {
      const displayValue = window.getComputedStyle(targetDiv).display;
      if (displayValue === "none") {
        console.log("div 被隐藏");
        First = true;
        if (First) {
          document.getElementById("offscreen-text").innerHTML = "";
          document.getElementById("text").innerHTML = "";
        }
        // 可以在这里添加更多的逻辑，例如触发动画或其他操作
      } else {
        if (First) {
          resizePage();
          addPrompt(0);
