  ...
  data() {
    ...
    return {
      ...
      option: {
        ...
        title: {
          text: this.$store.state.IndexData.CollectionSuccessRateTitle,
          // top: 10,
          // right: 10,
          // textStyle: {
          //     fontSize: 12,
            ...
            params.forEach(v => {
              html += `<div style='  font-size:16px;font-weight:600;'>
                ${v.axisValue}
                  ...
                  #4467EF;marign-right:5px"></span>当日代收成功率：${v.value * 100}%
                  </div></div>`
            })
            return html
          }
        ...
        series: [{
          name: this.$store.state.IndexData.CollectionSuccessRateTitle,
          type: 'line',
          smooth: true,
          symbolSize: 6,
          lineStyle: lineStyle(0),
          areaStyle: areaStyle(0),
          data: this.$store.state.IndexData.CollectionSuccessRateY,
          label: {
            show: false,
            position: 'top',
