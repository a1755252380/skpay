<template>
  <el-select :value="modelValue" clearable ref="ClientSearch" :loading="ClientSearchLoading" v-on="$listeners"
    v-bind="$attrs" :remote-method="mchNumSearch" @change="handleChange" placeholder="请选择商户号">
    <el-option v-for="item in ClientSearchList" :key="item.id" :label="item.mch_num" :value="item.mch_num"></el-option>
  </el-select>
</template>
